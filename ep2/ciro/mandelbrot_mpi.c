...
void compute_mandelbrot(int x0, int xFim, int y0, int yMax){
    double z_x;
    double z_y;
    double z_x_squared;
    double z_y_squared;
...
int main(int argc, char *argv[]){
    ...
    sqrt_p = sqrt(p);
    //bin_p = int_to_int(p);
    int x0 = my_rank*image_size/sqrt_p;
    int xFim = (my_rank+1)*image_size/sqrt_p;
    int y0 = my_rank*image_size/sqrt_p;
    int yFim = (my_rank+1)*image_size/sqrt_p;
    printf("rank %d   sqrt p %d   x0 %d   xFim %d   y0 %d   yFim %d \n", my_rank, sqrt_p, x0, xFim, y0, yFim);
